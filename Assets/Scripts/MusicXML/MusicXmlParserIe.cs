...
public class MusicXmlParserIe : MonoBehaviour
{
    static int nonChordPos = 0;
    static int chordtPos = 0;
    static float measureStartPost = 0;
    static int pmn = 100;
    private static Dictionary<int, Note> dictNoteTiedStart;


    public static IEnumerator GetScore(string filename)
    {
        Helpers.Score = null;
        Helpers.GetScoreCompleted = false;
        var timeStart = DateTime.Now;
        //Debug.Log(DateTime.Now.ToString("ss:ffff") + " GetScoreCompleted = false");

        pmn = 100;
        dictNoteTiedStart = new Dictionary<int, Note>();
        Measure measureLast = new Measure {Attributes = new MeasureAttributes {Time = new MusicXml.Domain.Time()}};
        ...
        yield return null;
        //Debug.Log($"IEnumerator GetScore Take {(DateTime.Now-timeStart).TotalMilliseconds.ToString("0000")}");
        Helpers.Score = score;
        Helpers.GetScoreCompleted = true;
    }
    ...
    private static Forward GetForwardElement(XmlNode node)
    {
        ...
        nonChordPos = nonChordPos + forward.Duration;
        chordtPos = nonChordPos;
    ...
    private static Backup GetBackupElement(XmlNode node)
    {
        ...
        nonChordPos = nonChordPos - backup.Duration;
        chordtPos = nonChordPos;
    ...
    private static Note GetNote(XmlNode noteNode)
    {
        ...
        var notationsNote = noteNode.SelectSingleNode("notations");
        if (notationsNote != null)
        {
            var arpeggiate = notationsNote.SelectSingleNode("arpeggiate");
            if (arpeggiate != null)
                note.IsArpeggiate = true;

            var technical = notationsNote.SelectSingleNode("technical");
            if (technical != null)
            {
                var fingering = technical.SelectSingleNode("fingering");
                int finger;
                if (fingering != null && int.TryParse(fingering.InnerText,
                        NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out finger))
                {
                    note.Finger = finger;
                }
            }
            ...
            var tiedNote = notationsNote.SelectSingleNode("tied");
            if (tiedNote != null
                && tiedNote.Attributes != null)
            {
                var tiedNoteType = tiedNote.Attributes["type"];
                if (tiedNoteType != null)
                {
                    note.Tied = tiedNoteType.InnerText;
                    int noteIndex = Helpers.NoteIndex(note.Pitch, 0);
                    if (note.Tied == "start")
                    {
                        if (!dictNoteTiedStart.ContainsKey(noteIndex))
                        {
                            dictNoteTiedStart.Add(noteIndex, note);
                            //Debug.Log("DictNoteTiedStart add " + note.Pitch.Step + "" +
                            //          note.Pitch.Octave + "" + note.Pitch.Alter + " " + noteIndex);
                        }
                        else
                        {
                            dictNoteTiedStart[noteIndex] = note;
                            //Debug.Log("DictNoteTiedStart Update " + note.Pitch.Step + "" +
                            //          note.Pitch.Octave + "" + note.Pitch.Alter + " " + noteIndex);
                        }
                    }
                    ...
                    if (note.Tied == "stop")
                    {
                        if (dictNoteTiedStart.ContainsKey(noteIndex))
                        {
                            dictNoteTiedStart[noteIndex].Duration =
                                dictNoteTiedStart[noteIndex].Duration + note.Duration;
                        }
                        else
                        {
                            Debug.LogWarning("Can not find dictNoteTiedStart for " + note.Pitch.Step + "" +
                                             note.Pitch.Octave + "" + note.Pitch.Alter + " " + noteIndex);
                        }
            ...
            //var tiedNotes = notationsNote.SelectNodes("tied");
            //if (tiedNotes != null)
            //{
            //    int i = 0;
            //    foreach (XmlNode tied in tiedNotes)
            //    {
            //        if (tied.Attributes != null)
            //        {
            //            var tiedNoteType = tied.Attributes["type"];
            //            Debug.Log(i + " tied: " + tiedNoteType.InnerText + " note.Tied " + note.Tied);
            //            i++;
            //        }
            //    }
            //}
    ...
    private static MusicXml.Domain.Time GetTime(XmlNode attributesNode)
    {
        var time = new MusicXml.Domain.Time();
    ...
    private static XmlDocument GetXmlDocumentContent(string fileContent)
    {
